...
export default function SwipePage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-50 via-white to-purple-50">
      <Navbar likeCount={likedIds.length} userName={userName} onLogout={handleLogout} />

      <main className="flex flex-col items-center px-4 py-8">
        {isDone ? (
          <div className="text-center py-24">
            <div className="text-8xl mb-6">🐾</div>
            <h2 className="text-3xl font-bold text-gray-800 mb-2">{"You've seen everyone!"}</h2>
            <p className="text-gray-500 mb-8">
              You liked {likedIds.length} pet{likedIds.length !== 1 ? 's' : ''} — great taste!
            </p>
            <div className="flex gap-4 justify-center flex-wrap">
              <Link href="/matches" className="bg-rose-500 text-white px-6 py-3 rounded-full font-semibold hover:bg-rose-600 transition-colors shadow-md">
                View Matches ({likedIds.length})
              </Link>
              <button onClick={reset} className="bg-white border-2 border-gray-200 text-gray-700 px-6 py-3 rounded-full font-semibold hover:border-gray-300 transition-colors">
                Start Over
              </button>
            </div>
          </div>
          ...
          <div className="w-full max-w-sm relative">
            {showMatch && (
              <div className="fixed inset-0 flex items-center justify-center z-50 pointer-events-none">
                <div className="bg-rose-500 text-white text-3xl font-black px-10 py-6 rounded-3xl shadow-2xl animate-bounce">
                  {"It's a Match! 🎉"}
                </div>
              </div>
            )}
