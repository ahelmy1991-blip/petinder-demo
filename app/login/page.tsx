...
export default function LoginPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(role === 'admin' ? '/admin' : role === 'provider' ? '/provider' : '/feed')
    } catch {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-rose-50 via-white to-purple-50 flex flex-col items-center justify-center px-4">
      ...
      <div className="w-full max-w-sm bg-white rounded-3xl shadow-xl p-8">
        ...
        <p className="text-gray-400 text-sm mb-6">Your pets missed you</p>
        ...
        <p className="text-center text-sm text-gray-400 mt-6">
          ...
          <Link href="/register" className="text-rose-500 font-semibold hover:underline">Sign up</Link>
        </p>
        <p className="text-center text-xs text-gray-300 mt-3">
          Demo: sara@petinder.app / admin@petinder.app · petinder123
        </p>
      </div>
    </div>
  )
